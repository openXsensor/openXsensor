...
void writeConf() {
  ...
  output.println("#define SENSOR_ID    " + cp5.get(DropdownList.class, "sensorID").captionLabel().getText()) ;
  output.println("") ;
  ...
  if ( ( cp5.getController("vario").value() == 1 || cp5.getController("airSpeed").value() == 1 ) && cp5.getController("ppm").value() == 1 ) {
    output.println("#define PIN_PPM         " + int(ppmPin.getValue()) + "       // Arduino can read a PPM Signal coming from Tx. This allows to change the vario sensitivity using a pot or a switch on TX.") ;
  } else {
  ...
  if ( cp5.getController("airSpeed").value() == 0 ) {
    output.print("//") ;
  }
  ...
  if ( cp5.getController("saveEprom").value() == 1 && cp5.getGroup("resetButtonPin").getValue() != -1 ) {

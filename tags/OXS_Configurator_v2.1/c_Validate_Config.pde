void validateNumPins() {
  ...
  String numPinsValidation[][] = new String[][] {                      // array { pin name, pin value, isActive }
    ...
    { "PPM input", "" + int(ppmPin.getValue()), "" + int( cp5.getController("ppm").getValue() + cp5.getController("vario").getValue() + cp5.getController("airSpeed").getValue() ) },
    { "Analog climb output", "" + int(climbPin.getValue()), "" + int( cp5.getController("analogClimb").getValue() + cp5.getController("vario").getValue() ) },
...
void validateAnalogPins() {
  ...
  String analogPinsValidation[][] = new String[][] {              // array { pin name, pin value, isActive }
    ...
    { "Vario/Air Speed (A4-A5)", "4", "" + int( cp5.getController("vario").getValue() + cp5.getController("airSpeed").getValue() ) },
    { "Vario/Air Speed (A4-A5)", "5", "" + int( cp5.getController("vario").getValue() + cp5.getController("airSpeed").getValue() ) }
  } ;
...
void validateSentData() {                // TODO better redundancy tests
  ...
  int oxsMeasureCountSPORT[][] = new int[sentDataList.length + 1][sPortDataList.length] ;               // array { sentData, sPortData }
    ...
    if ( cp5.getGroup("sentDataField" + i).getValue() > 0 ) {   // if OXS measurement field is not empty
      ...
      if ( oxsMeasureValidationList[i][4].equals("1") ) {                // if OXS measurement is valid (sensor available)
        if ( cp5.getGroup("protocolChoice").getValue() == 1 ) {          // if HUB protocol => maybe not up to date since OXSC v2.0
            ...
            if ( cp5.getGroup("sPortDataField" + i).getValue() == 0 ) {         // if telemetry field is empty
              sentDataValid = false ;
              messageList.append( "- The " + oxsMeasureValidationList[i][1] + " measure is not sent !" ) ;
                ...
                messageList.append( "- " + oxsMeasureValidationList[i][1] + " can't be set to DEFAULT !" ) ;
                          ...
                          && oxsMeasureValidationList[i][3].equals("DEFAULT") ) {
                for ( int j = i+1 ; j <= dataSentFieldNbr ; j++ ) {

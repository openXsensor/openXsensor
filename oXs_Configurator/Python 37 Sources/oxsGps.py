from oxsConfig import ttk , W , E , fGps , gps3dExist , gpsTimeExist , gpsRateVar
...
ttk.Label(fGps, text="3D speed" ).grid(column= 0, row=1, padx= (15,5), pady=5 , sticky=(E))
ttk.Checkbutton(fGps,  variable=gps3dExist,
	    onvalue='On', offvalue='Off').grid(column=1, row=1, sticky=('W','E') )
ttk.Label(fGps, text="GET UTCTIME (S-Port only)" ).grid(column= 0, row=2, padx= (15,5), pady=5 , sticky=(E))
ttk.Checkbutton(fGps,  variable=gpsTimeExist,
	    onvalue='On', offvalue='Off').grid(column=1, row=2, sticky=('W','E') )        
ttk.Label(fGps, text="Refresh rate (Hz)").grid(column= 0, row=3, padx= (15,5), pady=5 , sticky=(E))
gpsRateBox = ttk.Combobox(fGps, textvariable=gpsRateVar, 
                values=('1', "5" , "10"),
                state="readonly", width=2 )
gpsRateBox.grid(column=1, row=3, sticky=(W) )

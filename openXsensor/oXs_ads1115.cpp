...
extern void delay(unsigned long ms) ;
  ...
  const uint8_t ADS1115_MEASURE[4] = {ADS1115_MEASURE} ; //  how to configure the multiplexer
...
void OXS_ADS1115::ads_requestNextConv(void) {
    ...
    } while (  ADS1115_MEASURE[ads_CurrentIdx] == ADS_OFF ) ;
// perhaps this line has to be splitted in 2 in order to let multiplexer, gain and rate to set up before asking for a conversion.
    ...
    dataToWrite[0] = (( 1 << 7 | ADS1115_MEASURE[ads_CurrentIdx] << 4 | ads_Gain[ads_CurrentIdx] << 1 | 1  ) ) ;

...
uint8_t frskyStatus ;                     //Status of SPORT protocol saying is data is to load in work field (one value to transmit per sensor)
uint8_t currFieldIdx[6] ;                    // per sensor, say, which field has been loaded the last time (so next time, we have to search from the next one)
uint8_t fieldMinIdx[7] ;                    // per sensor, say the first field index ; there is one entry more in the array to know the last index
uint8_t fieldID[20] = { 0x10 , 0x11 , 0x30 , 0x30 , 0x30 , 0x21 , 0x20 , 0x60 , 0x80, 0x80 , 0x82 , 0x83 , 0x84 , 0x50 , 0x40 , 0x41 , 0xA0 , 0x70 , 0x71 , 0x72 } ; //fieldID to send to Tx (to shift 4 bits to left
struct ONE_MEASUREMENT * p_measurements[20] ;      // array of 20 pointers (each pointer point to a structure containing a byte saying if a value is available and the value.
...
if ( frskyStatus ) {                                                                                  // if at least one data has to be loaded
  for (uint8_t sensorSeq = 0 ; sensorSeq < 6 ; sensorSeq++ ) {                                        // for each sensor (currently 6)  
    if ( frskyStatus & (1<<sensorSeq] )  {                          //if frskyStatus says that a data must be loaded 
        ...
        for (unit8 iCount = fieldMinIdx[sensorSeq] ; iCount < fieldMinIdx[sensorSeq+1] ; iCount++ ) {        // we will not seach more than the number of fields for the selected sensor 
